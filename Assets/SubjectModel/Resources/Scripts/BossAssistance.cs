...
namespace SubjectModel
{
    ...
    public class BossAssistance : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (boss != null)
            {
                var health = (float) boss.GetComponent<Variables>().declarations.GetDeclaration("Health").value;
        ...
        public void StartBossFight(GameObject boss, RecoverAfterFight raf)
        {
            ...
            this.boss = boss;
            maxHealth = (float) boss.GetComponent<Variables>().declarations.GetDeclaration("Health").value;
            this.raf += raf;
            fighting = true;
        }
