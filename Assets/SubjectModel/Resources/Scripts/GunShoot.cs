    ...
    public class GunShoot : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (loading <= .0f)
            {
                ...
                {
                    switchingMagazine = false;
                    GetComponent<Variables>().declarations.Set("Speed",
                        GetComponent<Variables>().declarations.Get<float>("Speed") /
                        firearm.Data[FirearmComponent.ReloadSpeed]);
                    bulletRemain = (int) firearm.Data[FirearmComponent.Bullet];
                }
            }
        ...
        public void SwitchMagazine()
        {
            if (switchingMagazine) return;
            switchingMagazine = true;
            var loading = firearm.Data[FirearmComponent.Reload];
            GetComponent<Variables>().declarations.Set("Speed",
                GetComponent<Variables>().declarations.Get<float>("Speed") * firearm.Data[FirearmComponent.ReloadSpeed]);
            GetComponent<Variables>().declarations.Set("Loading", loading);
        }
        ...
        public void Shoot(Vector2 aim)
        {
            ...
            aim.x = Utils.GenerateGaussian(aim.x,
                firearm.Data[FirearmComponent.Deviation] * firearm.Data[FirearmComponent.Distance],
                firearm.Data[FirearmComponent.MaxRange]);
            aim.y = Utils.GenerateGaussian(aim.y,
                firearm.Data[FirearmComponent.Deviation] * firearm.Data[FirearmComponent.Distance],
                firearm.Data[FirearmComponent.MaxRange]);
            ...
            bulletRemain--;
            if (bulletRemain != 0)
                GetComponent<Variables>().declarations.Set("Loading", firearm.Data[FirearmComponent.Loading]);
            ...
            variables.declarations.Set("Health",
                health - (defence > firearm.Data[FirearmComponent.Depth]
                    ? Utils.Map(.0f, defence, .0f, firearm.Data[FirearmComponent.Damage],
                        firearm.Data[FirearmComponent.Depth])
                    : firearm.Data[FirearmComponent.Damage]));
        }
        ...
        private RaycastHit2D DrawLine(Vector2 from, Vector2 to, Pair<Color> success, Pair<Color> fault)
        {
            ...
            var hit = Physics2D.Raycast(origin, to - origin, firearm.Data[FirearmComponent.Distance], ShootMask);
            var dist = hit.collider == null ? firearm.Data[FirearmComponent.Distance] : hit.distance;
